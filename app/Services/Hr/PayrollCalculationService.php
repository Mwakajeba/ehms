...
class PayrollCalculationService
{
    ...
    public function calculateEmployeePayroll(Employee $employee, $payrollYear, $payrollMonth, $companyId = null)
    {
        ...
        $statutoryDeductions = $this->calculateStatutoryDeductions($employee, $salaryStructure, $earnings['gross'], $earnings['taxable_gross'], $startDate, $companyId);
    ...
    protected function calculateStatutoryDeductions(Employee $employee, $salaryStructure, $grossSalary, $taxableIncome, $date, $companyId)
    {
        ...
        $ageExemption = $this->calculateAgeBasedExemption($employee, $date);
        if ($ageExemption > 0) {
        ...
        if ($pensionRule && $employee->has_pension) {
    ...
    protected function calculateEmployerContributions(Employee $employee, $salaryStructure, $grossSalary, $nhifEmployee, $pensionEmployee, $wcf, $sdl, $date, $companyId)
    {
        ...
        $nhifRule = StatutoryRule::getActiveRule($companyId, StatutoryRule::TYPE_NHIF, $date);
        if ($nhifRule && $employee->has_nhif) {
            ...
            if ($percent > 0) {
                ...
                // If no employer percent specified, calculate based on employee contribution ratio
                // This is a fallback - ideally employer percent should be set in statutory rules
                $nhifEmployer = $nhifEmployee * 0.1; // Default 10% of employee contribution
            }
        }

        // Calculate Pension Employer Contribution
        $pensionRule = StatutoryRule::getActiveRule($companyId, StatutoryRule::TYPE_PENSION, $date);
        if ($pensionRule && $employee->has_pension) {
            // Use employee-specific employer percent if provided, otherwise use statutory
        ...
        return [
            'nhif_employer' => round($nhifEmployer, 2),
     ...
     * 5. date_end_of_loan (for loans) - Stops deduction after end date
     */
    protected function getOtherDeductions(Employee $employee, $year, $month, $companyId)
    {
        ...
        $advanceQuery = SalaryAdvance::where('employee_id', $employee->id)
            ->where('company_id', $companyId)
            ->where('is_active', true)
            ->where('date', '<=', $payrollDate);

        $advances = $advanceQuery->sum('monthly_deduction');
        ...
        $loanQuery = ExternalLoan::where('employee_id', $employee->id)
            ->where('company_id', $companyId)
            ->where('is_active', true)
            ->where('date', '<=', $payrollDate)
        ...
        $loans = $loanQuery->sum('monthly_deduction');

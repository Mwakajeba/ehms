...
class InventoryStockService
{
    ...
    public function getLowStockItemsAtLocation($locationId)
    {
        return Movement::where('location_id', $locationId)
            ...
            ->groupBy('item_id')
            ->having('current_stock', '>', 0)
            ->with(['item:id,name,code,reorder_level,minimum_stock,unit_of_measure'])
            ->get()
            ->filter(function ($movement) {
                $item = $movement->item;
                $currentStock = (float) $movement->current_stock;
                $reorderLevel = $item->reorder_level ?? 0;
                $minimumStock = $item->minimum_stock ?? 0;
                
                return $currentStock <= $reorderLevel || $currentStock <= $minimumStock;
            })
            ->map(function ($movement) {
                $item = $movement->item;
                $currentStock = (float) $movement->current_stock;
                ...
                return [
                    ...
                    'unit_of_measure' => $item->unit_of_measure,
                    'status' => $currentStock <= ($item->minimum_stock ?? 0) ? 'out_of_stock' : 'low_stock'
                ];
            })
            ->values();
    }

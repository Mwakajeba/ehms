...
class LipishaService
{
    ...
    public static function getControlNumberForInvoice($student, $amount, $period = null, $academicYearId = null, $invoiceNumber = null, $description = null)
    {
        ...
        if ($customerId && $customerId > 0) {
            ...
            // Check if invoice already has control number for this quarter
            if ($period && $academicYearId) {
                $existingInvoice = \App\Models\FeeInvoice::where('student_id', $student->id)
                    ->where('period', $period)
                    ->where('academic_year_id', $academicYearId)
                    ->whereNotNull('lipisha_control_number')
                    ->where('lipisha_control_number', '!=', '')
                    ->first();
                
                if ($existingInvoice && !empty($existingInvoice->lipisha_control_number)) {
                    Log::info('LIPISHA control number already exists for this quarter', [
                        'student_id' => $student->id,
                        'period' => $period,
                        'control_number' => $existingInvoice->lipisha_control_number
                    ]);
                    return $existingInvoice->lipisha_control_number;
                }
            }

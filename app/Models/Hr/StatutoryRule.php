...
class StatutoryRule extends Model
{
    ...
    /**
     * Get active rule for a type, date, and employee (with category matching)
     */
    public static function getActiveRuleForEmployee($companyId, $ruleType, Employee $employee, $date = null): ?self
    {
        $date = $date ?? now();
        
        // First, try to find category-specific rules
        $categoryRules = self::where('company_id', $companyId)
            ->where('rule_type', $ruleType)
            ->where('is_active', true)
            ...
            ->with('employeeCategories')
            ->orderBy('effective_from', 'desc')
            ->get();

        // Check if employee matches any category-specific rule

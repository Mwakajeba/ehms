...
class Retirement extends Model
{
    ...
    // Permission methods (using same approval settings as imprest)
    public function canUserCheck(User $user): bool
    {
        ...
        $approvalSetting = ImprestApprovalSetting::where('user_id', $user->id)
            ->where('approval_role', 'checker')
        ...
        // Check amount limit against retirement total
        if (!$approvalSetting->canApproveAmount($this->total_amount_used)) {
            return false;
        }

        // Check department access against imprest request department
        if (!$approvalSetting->canHandleDepartment($this->imprestRequest->department_id)) {
            return false;
        }

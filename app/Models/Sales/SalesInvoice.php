...
class SalesInvoice extends Model
{
    ...
    public function createDoubleEntryTransactions()
    {
        ...
        foreach ($this->items as $item) {
            ...
            if ($item->item_code === 'TRANSPORT') {
                ...
                $salesRevenueAmount += $netLine;
            }
        }
        
        // Credit Sales Revenue for product sales
        if ($salesRevenueAmount > 0) {
            $salesRevenueAmountLCY = $convertToLCY($salesRevenueAmount);
            $transactions[] = [
                'chart_account_id' => $this->getSalesAccountId(),
                'customer_id' => $this->customer_id,
                'amount' => $salesRevenueAmountLCY,
                'nature' => 'credit',
                'transaction_id' => $this->id,
                'transaction_type' => 'sales_invoice',
                'date' => $this->invoice_date,
    ...
    public function recordBankPayment($amount, $paymentDate, $bankAccountId, $description, $whtTreatment = null, $whtRate = 0, $vatMode = 'EXCLUSIVE', $vatRate = 18, $paymentExchangeRate = null)
    {
        ...
        // Pass the full receipt amount (net_receivable) to credit Trade Receivables correctly
        // The net_receivable is the amount that should be credited to Trade Receivables
        $this->createBankPaymentGlTransactions($receipt, $receiptNetReceivable, $paymentDate, $description, $receiptWHT, $whtTreatment, $paymentRate);
    ...
    {
        return $this->hasMany(Receipt::class, 'reference', 'id')
    ...
    public function reversePayment($receiptId, $reason = null)
    {
        ...
        if ($receipt->reference != $this->id || $receipt->reference_type != 'sales_invoice') {
     ...
     */
    private function syncLinkedOpeningBalance(): void
    {
    ...
    public function getAmountInWords()
    {
        ...
        for ($i = 0; $amount > 0; $i++) {
            $group = $amount % 1000;
            if ($group != 0) {
                $groupWords = $this->convertGroup($group, $ones, $tens);
                $result = $groupWords . ' ' . $thousands[$i] . ' ' . $result;
            }
    ...
    private function convertGroup($num, $ones, $tens)
    {
        $result = '';

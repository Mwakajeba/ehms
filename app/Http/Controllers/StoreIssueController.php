    ...
    public function store(Request $request)
    {
        ...
        try {
            $request->validate([
                ...
                'items.*.quantity_issued.required' => 'Quantity issued is required for each item.',
        ...
        // Verify the location belongs to user's company and branch
        ...
        foreach ($request->items as $item) {
            ...
            try {
                ...
                if ($currentStock <= 0) {
                    \Log::error('Item not available at selected location', [
                    ...
                    return back()->withInput()->withErrors([
                        'error' => "Item '{$requisitionItem->product->name}' is not available at {$location->name}."
                    ]);
                }
                
                if ($currentStock < $item['quantity_issued']) {
                    \Log::error('Insufficient stock at selected location', [
                        'item_name' => $requisitionItem->product->name,
                        'location_id' => $selectedLocationId,
                        'location_name' => $location->name,
                        'available_stock' => $currentStock,
                        'required_quantity' => $item['quantity_issued']
                    ]);
                    return back()->withInput()->withErrors([
                        'error' => "Insufficient stock for {$requisitionItem->product->name} at {$location->name}. Available: {$currentStock}, Required: {$item['quantity_issued']}"
                    ]);
                }

                \Log::info('Stock validation passed at selected location', [
                    'item_name' => $requisitionItem->product->name,
                    'location_id' => $selectedLocationId,
                    'location_name' => $location->name,
                    'available_stock' => $currentStock,
                ...
                \Log::error('Failed to validate stock at selected location', [
                    'item_id' => $requisitionItem->id,
                    'location_id' => $selectedLocationId,
                    'error' => $e->getMessage()
                ]);
        ...
        try {
            ...
            $totalAmount = 0;
            foreach ($request->items as $item) {
                $requisitionItem = StoreRequisitionItem::find($item['requisition_item_id']);
                $totalAmount += $requisitionItem->unit_cost * $item['quantity_issued'];
            }
            ...
            // Update requisition items with issued quantities and create inventory movements
            $allItemsFullyIssued = true;
            foreach ($request->items as $item) {
                $requisitionItem = StoreRequisitionItem::find($item['requisition_item_id']);
                $newQuantityIssued = $requisitionItem->quantity_issued + $item['quantity_issued'];
                ...
                // Note: GL transactions are now handled within createInventoryMovement method
            }
            ...
            return redirect()->route('store-issues.show', $storeIssue->id)
                ->with('success', 'Store issue created successfully. Voucher No: ' . $voucherNo);

...
class StoreRequisitionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // First check if the employee has a department assigned
            $departmentId = null;
            if ($employeeData && $employeeData->department_id) {
                $departmentId = $employeeData->department_id;
            } else {
                // Use default department (get first available department for the company)
                $defaultDepartment = Department::where('company_id', $user->company_id)->first();
                if (!$defaultDepartment) {
                    throw new \Exception('No departments found. Please create a department first.');
                }
            ...
            return redirect()->route('store-requisitions.requisitions.show', $requisition->hash_id)
                ->with('success', 'Store requisition created successfully. Voucher No: ' . $voucherNo);

        } catch (\Exception $e) {
            DB::rollback();
            \Log::error('Store Requisition Creation Failed: ' . $e->getMessage());
            \Log::error('Stack trace: ' . $e->getTraceAsString());
            return back()->withInput()->withErrors(['error' => 'Failed to create store requisition. Error: ' . $e->getMessage()]);
        }
    }

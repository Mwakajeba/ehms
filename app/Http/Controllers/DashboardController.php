...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Total Sales Today = Invoices + Cash Sales + POS Sales
        $totalSalesToday = $invoicesTodayTotal + $cashSalesTodayTotal + $posSalesTodayTotal;
        ...
        // Total Expenses Today (payments) — payments table has no company_id column
        $totalExpensesToday = \App\Models\Payment::when(!empty($permittedBranchIds), fn($q) => $q->whereIn('branch_id', $permittedBranchIds))
            ->when($branchId, fn($q) => $q->where('branch_id', $branchId))
    ...
    private function getFinancialReportData($branchId = null, array $permittedBranchIds = [], $balanceSheetEndDate = null, $incomeStatementEndDate = null, $incomeStatementStartDate = null)
    {
        ...
        $groupByFields = [
            'chart_accounts.id', 'chart_accounts.account_name', 'chart_accounts.account_code', 
        ...
        // Combine results
        $chartAccountsData = $balanceSheetData->merge($incomeStatementData);
    ...
    private function getPreviousYearData($branchId = null, array $permittedBranchIds = [], $balanceSheetEndDate = null, $incomeStatementStartDate = null, $incomeStatementEndDate = null)
    {
        ...
        // Combine results
        $previousYearData = $balanceSheetData->merge($incomeStatementData);
    ...
    private function getExpiringItems($branchId = null, array $permittedBranchIds = [])
    {
        ...
        // Get warning days from system settings (default 30 days)
        ...
        // Get expiring items from expiry tracking
        $expiringItems = \App\Models\Inventory\ExpiryTracking::with(['item', 'location'])
            ...
            ->whereHas('location', function($query) use ($permittedBranchIds, $branchId) {
                if (!empty($permittedBranchIds)) {
            ...
            ->map(function($tracking) {
                $daysUntilExpiry = (int) abs(now()->diffInDays($tracking->expiry_date));
                return [
                    'id' => $tracking->id,
                    'item_name' => $tracking->item->name,
                    'item_code' => $tracking->item->item_code,
                    'batch_number' => $tracking->batch_number,
                    'expiry_date' => $tracking->expiry_date,
                    'available_quantity' => $tracking->quantity,
                    'location_name' => $tracking->location->name,

...
class PayrollController extends Controller
{
    ...
    private function calculateEmployeePayroll(Employee $employee, Payroll $payroll)
    {
        ...
        try {
            ...
            $externalLoans = $employee->externalLoans
                ->where('is_active', true)
    ...
    private function createPayrollAccrualJournalEntry(Payroll $payroll)
    {
        ...
        // Calculate totals
        $totals = $payroll->payrollEmployees->reduce(function ($carry, $employee) use ($payroll, $companyId) {
            // Calculate employer pension contribution
            $employerPension = 0;
            if ($employee->employee && $employee->employee->has_pension) {
                $pensionRule = \App\Models\Hr\StatutoryRule::getActiveRule($companyId, \App\Models\Hr\StatutoryRule::TYPE_PENSION, Carbon::create($payroll->year, $payroll->month, 1));
                $grossSalary = $employee->gross_salary;
                
                if ($pensionRule && $pensionRule->pension_employer_percent) {
                    $base = $grossSalary;
                    if ($pensionRule->pension_ceiling) {
                        $base = min($base, $pensionRule->pension_ceiling);
                    }
                    $employerPension = $base * ($pensionRule->pension_employer_percent / 100);
                } elseif ($employee->employee->pension_employer_percent) {
                    $employerPension = $grossSalary * ($employee->employee->pension_employer_percent / 100);
                }
            }
            ...
            'insurance', 'salary_advance', 'loans', 'trade_union', 'sdl', 
        ...
        $netSalary = $grossSalary - ($totals['paye'] + $totals['pension'] + $totals['insurance'] + 
                     ...
                     $totals['sdl'] + $totals['wcf'] + $totals['heslb'] + $totals['other_deductions']);

        // 3. CR Salary Payable Account (Net amount to pay to employees)
        ...
        // 4. CR Employee Deduction Payables (amounts deducted from employee salaries)
        ...
        // 5. DR & CR Employer Statutory Contributions (separate transactions)
    ...
    private function addEmployeeDeductionPayables($journal, &$journalItems, &$glTransactions, $chartAccounts, $totals, $payroll, $companyId, $branchId, $userId, $journalDate)
    {
        $deductionPayables = [
            ...
            // Note: Pension Payable is handled in addEmployerStatutoryContributions() to credit with TOTAL (Employee + Employer)
            ['amount' => $totals['insurance'], 'account_id' => $chartAccounts->insurance_payable_account_id, 'description' => 'Insurance payable (Employee NHIF)'],
    ...
    private function addEmployerStatutoryContributions($journal, &$journalItems, &$glTransactions, $chartAccounts, $totals, $payroll, $companyId, $branchId, $userId, $journalDate)
    {
        ...
        // WCF (Workers Compensation Fund) - Employee deductions + Employer contribution
        $wcfEmployee = $totals['wcf'] ?? 0; // Employee WCF deductions
        $wcfEmployer = $totals['totalWcf'] ?? $payroll->total_wcf ?? 0; // Employer WCF contribution
        $totalWcf = $wcfEmployee + $wcfEmployer;
        
        if ($wcfEmployer > 0 && $chartAccounts->wcf_expense_account_id && $chartAccounts->wcf_payable_account_id) {
            // DR WCF Expense (Employer portion only)
            $this->addJournalItemAndGLTransaction(
                $journal, $journalItems, $glTransactions,
        ...
        // CR WCF Payable (Total: Employee + Employer)
        if ($totalWcf > 0 && $chartAccounts->wcf_payable_account_id) {
            $this->addJournalItemAndGLTransaction(
                $journal, $journalItems, $glTransactions,
                $chartAccounts->wcf_payable_account_id,
                $totalWcf, 'credit',
                'WCF payable (Total: Employee + Employer) for ' . $payroll->month . '/' . $payroll->year,
                $companyId, $branchId, $userId, $journalDate, $payroll->id, 'payroll_accrual'
            );
        }

        // SDL (Skills Development Levy) - Employer contribution
        if ($totals['sdl'] > 0 && $chartAccounts->sdl_expense_account_id && $chartAccounts->sdl_payable_account_id) {
            // DR SDL Expense
            $this->addJournalItemAndGLTransaction(
                $journal, $journalItems, $glTransactions,
        ...
        // NHIF (National Health Insurance Fund) - Employer contribution
        // Note: $totals['totalNhifEmployer'] comes from payroll totals, not from employee-level calculation
        $nhifEmployer = $payroll->total_nhif_employer ?? 0;
        if ($nhifEmployer > 0 && $chartAccounts->insurance_expense_account_id && $chartAccounts->insurance_payable_account_id) {
            // DR NHIF Expense (Employer portion)
            $this->addJournalItemAndGLTransaction(
                $journal, $journalItems, $glTransactions,
                $chartAccounts->insurance_expense_account_id,
                $nhifEmployer, 'debit',
                'NHIF expense (Employer) for ' . $payroll->month . '/' . $payroll->year,
                $companyId, $branchId, $userId, $journalDate, $payroll->id, 'payroll_accrual'
            );
            
            // CR NHIF Payable (Employer portion)
            $this->addJournalItemAndGLTransaction(
                $journal, $journalItems, $glTransactions,
                $chartAccounts->insurance_payable_account_id,
                $nhifEmployer, 'credit',
                'NHIF payable (Employer) for ' . $payroll->month . '/' . $payroll->year,
                $companyId, $branchId, $userId, $journalDate, $payroll->id, 'payroll_accrual'
            );
        }
    }
    ...
    private function recordAdvanceAndLoanRepayments(Payroll $payroll)
    {
        ...
        foreach ($payrollEmployees as $payrollEmployee) {
            ...
            if ($payrollEmployee->salary_advance > 0) {
                try {
                    // Get all active advances that should be deducted in this payroll
                    ...
                    foreach ($advances as $advance) {
                        // Calculate total deductions made for this specific advance
                        // Count how many payroll periods this advance has been deducted
                        // A period counts if: advance date <= payroll date AND employee had salary_advance deduction
                        $payrollPeriods = \App\Models\Payroll::where('company_id', $payroll->company_id)
                            ...
                            ->get()
                            ->filter(function ($p) use ($advance) {
                                $pDate = Carbon::create($p->year, $p->month, 1)->endOfMonth();
                                // Advance must have been created before or on this payroll date
                                return $advance->date <= $pDate;
                            })
                            ->count();

                        // Calculate total deductions = number of periods * monthly deduction
                        $totalDeductions = $payrollPeriods * $advance->monthly_deduction;

                        // Calculate remaining balance
                        $remainingBalance = max(0, $advance->amount - $totalDeductions);

                        // Auto-deactivate if balance is 0 or less
                        if ($remainingBalance <= 0) {
                            $advance->is_active = false;
                            $advance->save();
                            \Log::info("Auto-deactivated salary advance ID {$advance->id} for employee {$employee->id} - Balance reached 0 (Original: {$advance->amount}, Deducted: {$totalDeductions})");
                        }
                    }
                } catch (\Exception $e) {
            ...
            if ($payrollEmployee->loans > 0) {
                try {
                    ...
                    foreach ($loans as $loan) {
                        ...
                        // Count how many payroll periods this loan has been deducted
                        $payrollPeriods = \App\Models\Payroll::where('company_id', $payroll->company_id)
                            ->where('id', '<=', $payroll->id)
                            ->whereHas('payrollEmployees', function ($q) use ($employee) {
                                $q->where('employee_id', $employee->id)
                        ...
                        if ($remainingBalance <= 0) {
                            ...
                            $loan->save();
                            \Log::info("Auto-deactivated external loan ID {$loan->id} for employee {$employee->id} - Balance reached 0 (Original: {$loan->total_loan}, Deducted: {$totalDeductions})");
                        }
                    }
                } catch (\Exception $e) {

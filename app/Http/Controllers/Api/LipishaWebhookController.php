    ...
    public function handle(Request $request)
    {
        try {
            ...
            if ($data) {
                ...
                // Find the invoice by bill_number (control number)
                $invoice = null;
                if ($billNumber) {
                    $invoice = FeeInvoice::where('lipisha_control_number', $billNumber)
                        ->where('status', '!=', 'cancelled')
                        ->first();
                }
                
                // Fallback: try to find by invoice number if bill_number not found
                if (!$invoice && isset($data['invoice'])) {
                    $invoice = FeeInvoice::where('invoice_number', $data['invoice'])
                        ->where('status', '!=', 'cancelled')
                        ->first();
                }

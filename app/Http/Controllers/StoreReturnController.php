...
class StoreReturnController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'items.*.quantity_returned.required' => 'Quantity returned is required for each item.',
            'items.*.quantity_returned.min' => 'Quantity returned must be greater than 0.',
        ]);
        ...
        // Validate quantities don't exceed issued amounts
        ...
        try {
            ...
            foreach ($request->items as $item) {
                $requisitionItem = StoreRequisitionItem::find($item['item_id']);
                $totalAmount += $requisitionItem->unit_price * $item['quantity_returned'];
            }
            ...
            foreach ($request->items as $item) {
                $requisitionItem = StoreRequisitionItem::find($item['item_id']);
                $newQuantityReturned = $requisitionItem->quantity_returned + $item['quantity_returned'];

...
class DeliveryController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('sales.deliveries.create', compact('order', 'customers', 'inventoryItems'))->with('items', $inventoryItems);
    }
    ...
    public function downloadDeliveryNotePdf($id)
    {
        ...
        $delivery = Delivery::with(['customer', 'items.inventoryItem'])
            ->when($user->branch_id, function ($query) use ($user) {
                return $query->where('branch_id', $user->branch_id);
            })
            ->findOrFail($decodedId);

        $company = optional($user->branch)->company ?? null;

        $html = view('sales.deliveries.note', compact('delivery', 'company'))->render();

...
use App\Jobs\ProcessSalesInvoiceItemsJob;
...
class SalesInvoiceController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        return view('sales.invoices.create', compact('customers', 'inventoryItems', 'salesOrders', 'selectedCustomer', 'currencies'))->with('items', $inventoryItems);
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($useJob) {
                \Log::info('Sales Invoice Store: Using job for large batch', [
                    'invoice_id' => $invoice->id,
                    'items_count' => $itemsCount
                ]);

                // Dispatch job to process items asynchronously
                ProcessSalesInvoiceItemsJob::dispatch(
                    $invoice->id,
                    $request->items,
                    $request->sales_order_id
                );

                \Log::info('Sales Invoice Store: Job dispatched', [
                    'invoice_id' => $invoice->id
                ]);
                ...
                if ($request->ajax()) {
                    return response()->json([
                        'success' => true,
                        'message' => 'Sales invoice created successfully. Items are being processed in the background. Please refresh the page in a few moments.',
                        'redirect_url' => route('sales.invoices.show', $invoice->encoded_id)
                    ]);
                }

                return redirect()->route('sales.invoices.show', $invoice->encoded_id)
                    ->with('success', 'Sales invoice created successfully. Items are being processed in the background. Please refresh the page in a few moments.');
            } else {
                ...
                if ($inventoryItem->track_stock && $inventoryItem->item_type === 'product') {
                    ...
                    $balanceBefore = $inventoryItem->current_stock;
            ...
            \Log::error('Sales Invoice Creation Error: ' . $e->getMessage());
            ...
            return redirect()->back()
                ->with('error', 'Failed to create sales invoice: ' . $e->getMessage())
                ->withInput();
        }
    }
    ...
    public function edit($encodedId)
    {
        ...
        return view('sales.invoices.edit', compact('invoice', 'customers', 'inventoryItems', 'salesOrders', 'currencies'))->with('items', $inventoryItems);
    }
    ...
    public function update(Request $request, $encodedId)
    {
        ...
        foreach ($existingItems as $existingItem) {
            ...
            if (!isset($stockToReturn[$itemId])) {
                $stockToReturn[$itemId] = 0;
            }
            $stockToReturn[$itemId] += $existingItem->quantity;
        }
        ...
        foreach ($request->items as $index => $itemData) {
            ...
            if ($inventoryItem && 
                $inventoryItem->item_type !== 'service' && 
                $inventoryItem->track_stock) {
                // Calculate available stock including stock that will be returned
                $availableStock = $inventoryItem->current_stock;
                if (isset($stockToReturn[$inventoryItem->id])) {
                    $availableStock += $stockToReturn[$inventoryItem->id];
                }
                
                if ($itemData['quantity'] > $availableStock) {
                    $validator->errors()->add("items.{$index}.quantity",
                        "Insufficient stock for {$inventoryItem->name}. Available: {$availableStock}, Requested: {$itemData['quantity']}");
                }
            }
        }
        ...
        try {
            ...
            if ($useJob) {
                ...
                \Log::info('Sales Invoice Update: Job dispatched', [
                    'invoice_id' => $invoice->id
                ]);
                ...
                return redirect()->route('sales.invoices.show', $invoice->encoded_id)
                    ->with('success', 'Sales invoice updated successfully. Items are being processed in the background. Please refresh the page in a few moments.');
            } else {
                // Process items synchronously for smaller batches
    ...
    public function deletePayment(Request $request, $encodedId)
    {
        ...
        try {
            ...
            // Verify the receipt belongs to this invoice
            if ($receipt->reference != $invoice->id || $receipt->reference_type != 'sales_invoice') {
    ...
    public function updateReceipt(Request $request, $encodedId)
    {
        ...
        try {
            ...
            if ($request->payment_method === 'bank') {
                ...
                // Pass net_receivable (full receipt amount) instead of base_amount
                // Pass payment exchange rate for FX gain/loss calculation
                $invoice->createBankPaymentGlTransactions($receipt, $netReceivable, $request->payment_date, $request->description ?: "Receipt for Invoice #{$invoice->invoice_number}", $whtAmount, $whtTreatment, $paymentExchangeRate);
            } else {
    ...
    public function exportPdf($encodedId)
    {
        ...
        try {
            $pdf = \PDF::loadView('sales.invoices.pdf', compact('invoice', 'unpaidInvoices', 'bankAccounts'));
            ...
            // Check if download parameter is present to force download instead of viewing

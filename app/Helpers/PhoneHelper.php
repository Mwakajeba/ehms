...
if (!function_exists('find_guardian_by_phone')) {
    ...
    function find_guardian_by_phone($phone)
    {
        ...
        // Try to find guardian with normalized phone number
        $guardian = \App\Models\School\Guardian::where('phone', $normalized)->first();
        ...
        // Try each variation
        foreach ($variations as $variation) {
            $guardian = \App\Models\School\Guardian::where('phone', $variation)->first();
            if ($guardian) {
                return $guardian;
            }
        }

...
// Schedule biometric device sync (runs every 15 minutes)
Schedule::command('biometric:sync')
    ->everyFifteenMinutes()
    ->description('Sync data from biometric devices that need sync')
    ->withoutOverlapping()
    ->runInBackground();

// Schedule biometric log processing (runs every 5 minutes)
Schedule::command('biometric:process-logs --limit=200')
    ->everyFiveMinutes()
    ->description('Process pending biometric logs and create attendance records')
    ->withoutOverlapping()
    ->runInBackground();

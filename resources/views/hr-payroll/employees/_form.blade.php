        ...
        <div class="form-section">
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_nhif" name="has_nhif" value="1" {{ old('has_nhif', $employee->has_nhif ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_nhif">Enable NHIF</label>
                    </div>
                </div>
                <div id="nhif_fields" class="row g-3 {{ old('has_nhif', $employee->has_nhif ?? false) ? '' : 'd-none' }}">
                    <div class="col-md-4">
                        <label class="form-label">Employee %</label>
                        <input type="number" step="0.01" name="nhif_employee_percent" class="form-control"
                            value="{{ old('nhif_employee_percent', $employee->nhif_employee_percent ?? '') }}" placeholder="e.g., 3.0">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Employer %</label>
                        <input type="number" step="0.01" name="nhif_employer_percent" class="form-control"
                            value="{{ old('nhif_employer_percent', $employee->nhif_employer_percent ?? '') }}" placeholder="e.g., 3.0">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">NHIF Member Number</label>
                        <input type="text" name="nhif_member_number" class="form-control"
                            value="{{ old('nhif_member_number', $employee->nhif_member_number ?? '') }}">
                    </div>
                </div>
            </div>
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_pension" name="has_pension" value="1" {{ old('has_pension', $employee->has_pension ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_pension">Enable Pension</label>
                    </div>
                </div>
                <div class="alert alert-info mb-2">
                    <i class="bx bx-info-circle me-1"></i>
                    <small><strong>Note:</strong> Pension rates are configured in Statutory Rules. Employee-specific rates below override the statutory rate if provided. After creating the employee, ensure you add a Pension compliance record.</small>
                </div>
                <div id="pension_fields" class="row g-3 {{ old('has_pension', $employee->has_pension ?? false) ? '' : 'd-none' }}">
                    <div class="col-md-4">
                        <label class="form-label">Social Fund Type {!! $required !!}</label>
                        <select name="social_fund_type" class="form-select select2-single" required>
                            <option value="">-- Select --</option>
                            <option value="PSSSF" @selected(old('social_fund_type', $employee->social_fund_type ?? '') === 'PSSSF')>
                                PSSSF (Public Service Social Security Fund)
                            </option>
                            <option value="NSSF" @selected(old('social_fund_type', $employee->social_fund_type ?? '') === 'NSSF')>
                                NSSF (National Social Security Fund)
                            </option>
                        </select>
                        <small class="text-muted">Select the pension scheme type</small>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Social Fund Number {!! $required !!}</label>
                        <input type="text" name="social_fund_number" class="form-control"
                            value="{{ old('social_fund_number', $employee->social_fund_number ?? '') }}" required
                            placeholder="e.g., PSSSF123456 or NSSF789012">
                        <small class="text-muted">Employee's pension membership number</small>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">
                            Employee % 
                            <small class="text-muted">(Optional - overrides statutory rate)</small>
                        </label>
                        <input type="number" step="0.01" min="0" max="100" name="pension_employee_percent" class="form-control"
                            value="{{ old('pension_employee_percent', $employee->pension_employee_percent ?? '') }}" 
                            placeholder="e.g., 5.0">
                        <small class="text-muted">
                            <i class="bx bx-info-circle me-1"></i>
                            Leave empty to use rate from Statutory Rules. This overrides the statutory rate if provided.
                        </small>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">
                        ...
                        <input type="number" step="0.01" min="0" max="100" name="pension_employer_percent" class="form-control"
                            value="{{ old('pension_employer_percent', $employee->pension_employer_percent ?? '') }}" 
                            placeholder="e.g., 5.0">
                        <small class="text-muted">
                            <i class="bx bx-info-circle me-1"></i>
                            Employer contribution rate (for reference/documentation only, not used in employee deductions)
                        </small>
                    </div>
                    <div class="col-12">
                        <div class="alert alert-warning mb-0">
                            <i class="bx bx-error-circle me-1"></i>
                            <small><strong>Important:</strong> After creating this employee, go to <strong>Employee Compliance</strong> to add a Pension compliance record with the membership details and expiry date.</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_trade_union" name="has_trade_union" value="1" {{ old('has_trade_union', $employee->has_trade_union ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_trade_union">Enable Trade Union</label>
                    </div>
                </div>
                <div id="union_fields" class="row g-3 {{ old('has_trade_union', $employee->has_trade_union ?? false) ? '' : 'd-none' }}">
                    <div class="col-md-4">
                        <label class="form-label">Trade Union</label>
                        <div class="input-group">
                            <select name="trade_union_id" class="form-select select2-single" id="trade_union_select">
                                <option value="">-- Select Trade Union --</option>
                                @foreach($tradeUnions as $tradeUnion)
                                    <option value="{{ $tradeUnion->id }}" @selected(old('trade_union_id', $employee->trade_union_id ?? '') == $tradeUnion->id)>
                                        {{ $tradeUnion->name }}
                                    </option>
                                @endforeach
                            </select>
                            <button type="button" class="btn btn-outline-secondary" id="refresh_trade_unions" title="Refresh Trade Unions List">
                                <i class="bx bx-refresh"></i>
                            </button>
                            <a href="{{ route('hr.trade-unions.create') }}" target="_blank" class="btn btn-outline-primary" title="Create New Trade Union">
                                <i class="bx bx-plus"></i>
                            </a>
                        </div>
                        <small class="text-muted">Click <i class="bx bx-plus"></i> to create or <i class="bx bx-refresh"></i> to refresh list</small>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Category</label>
                        <select name="trade_union_category" class="form-select select2-single" id="trade_union_category"
                            onchange="updateTradeUnionInputLabel()">
                            <option value="">-- Select Category --</option>
                            <option value="amount" @selected(old('trade_union_category', $employee->trade_union_category ?? '') == 'amount')>Amount</option>
                            <option value="percentage" @selected(old('trade_union_category', $employee->trade_union_category ?? '') == 'percentage')>Percentage</option>
                        </select>
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" id="trade_union_value_label">Amount/Percentage</label>
                        <input type="number" step="0.01" name="trade_union_value" class="form-control" id="trade_union_value"
                            value="{{ old('trade_union_value', $employee->trade_union_amount ?? $employee->trade_union_percent ?? '') }}">
                    </div>
                </div>
            </div>
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_wcf" name="has_wcf" value="1" {{ old('has_wcf', $employee->has_wcf ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_wcf">Enable WCF</label>
                    </div>
                </div>
                <div id="wcf_fields" class="row g-3 {{ old('has_wcf', $employee->has_wcf ?? false) ? '' : 'd-none' }}">
                    <div class="col-md-6">
                        <label class="form-label">Employee %</label>
                        <input type="number" step="0.01" name="wcf_employee_percent" class="form-control"
                            value="{{ old('wcf_employee_percent', $employee->wcf_employee_percent ?? '') }}" placeholder="e.g., 1.0">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Employer %</label>
                        <input type="number" step="0.01" name="wcf_employer_percent" class="form-control"
                            value="{{ old('wcf_employer_percent', $employee->wcf_employer_percent ?? '') }}" placeholder="e.g., 1.0">
                    </div>
                </div>
            </div>
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_heslb" name="has_heslb" value="1" {{ old('has_heslb', $employee->has_heslb ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_heslb">Has HESLB Loan</label>
                    </div>
                </div>
                <div class="alert alert-info mb-2">
                    <i class="bx bx-info-circle me-1"></i>
                    <small><strong>Note:</strong> HESLB deductions are automatically applied to employees with active loan balances. The deduction rate is set in the Statutory Rules. After creating the employee, go to HESLB Loans to create a loan record with the loan balance.</small>
                </div>
                <div id="heslb_fields" class="row g-3 d-none">
                    <div class="col-12">
                        <div class="alert alert-warning mb-0">
                            <i class="bx bx-info-circle me-1"></i>
                            <small>This checkbox indicates the employee has a HESLB loan. You must create a loan record in <strong>HESLB Loans</strong> with the loan balance for deductions to be calculated.</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="benefit-card">
                <div class="benefit-header">
                    ...
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="has_sdl" name="has_sdl" value="1" {{ old('has_sdl', $employee->has_sdl ?? false) ? 'checked' : '' }}>
                        <label class="form-check-label" for="has_sdl">Enable SDL</label>
                    </div>
                </div>
                <div id="sdl_fields" class="row g-3 {{ old('has_sdl', $employee->has_sdl ?? false) ? '' : 'd-none' }}">
                    <div class="col-md-6">
                        <label class="form-label">Employee %</label>
                        <input type="number" step="0.01" name="sdl_employee_percent" class="form-control"
                            value="{{ old('sdl_employee_percent', $employee->sdl_employee_percent ?? '') }}" placeholder="e.g., 0.5">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Employer %</label>
                        <input type="number" step="0.01" name="sdl_employer_percent" class="form-control"
                            value="{{ old('sdl_employer_percent', $employee->sdl_employer_percent ?? '') }}" placeholder="e.g., 0.5">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    // Function to update trade union input label based on category selection
    function updateTradeUnionInputLabel() {
        const categorySelect = document.getElementById('trade_union_category');
        const label = document.getElementById('trade_union_value_label');
        const input = document.getElementById('trade_union_value');

        if (categorySelect && label && input) {
            const selectedValue = categorySelect.value;
            if (selectedValue === 'amount') {
                label.textContent = 'Amount (TZS)';
                input.placeholder = 'Enter amount in TZS';
            } else if (selectedValue === 'percentage') {
                label.textContent = 'Percentage (%)';
                input.placeholder = 'Enter percentage (e.g., 2.5)';
            } else {
                label.textContent = 'Amount/Percentage';
    ...
    // Function to refresh trade unions list
    function refreshTradeUnions() {
        const select = document.getElementById('trade_union_select');
        const refreshBtn = document.getElementById('refresh_trade_unions');
        ...
        fetch('{{ route("hr.trade-unions.ajax.list") }}', {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            ...
            select.innerHTML = '<option value="">-- Select Trade Union --</option>';
            
            // Add new options
            data.forEach(tradeUnion => {
                const option = document.createElement('option');
                option.value = tradeUnion.id;
                option.textContent = tradeUnion.name;
                if (currentValue == tradeUnion.id) {
        ...
        .catch(error => {
            console.error('Error refreshing trade unions:', error);
            alert('Failed to refresh trade unions list. Please try again.');
            refreshBtn.disabled = false;
            refreshBtn.innerHTML = '<i class="bx bx-refresh"></i>';
        });
    }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Setup refresh button for trade unions
        const refreshBtn = document.getElementById('refresh_trade_unions');
        if (refreshBtn) {
            refreshBtn.addEventListener('click', refreshTradeUnions);
        }
        
        // Listen for focus on window (when user comes back from creating trade union)
        window.addEventListener('focus', function() {
            // Optionally auto-refresh when window regains focus
            // Uncomment the line below if you want auto-refresh on focus
            // refreshTradeUnions();
        });
        
        updateTradeUnionInputLabel();

        // Set the correct value based on existing data when editing
        @if(isset($employee))
            @if($employee->trade_union_amount)
                document.getElementById('trade_union_category').value = 'amount';
                document.getElementById('trade_union_value').value = '{{ $employee->trade_union_amount }}';
            @elseif($employee->trade_union_percent)
                document.getElementById('trade_union_category').value = 'percentage';
                document.getElementById('trade_union_value').value = '{{ $employee->trade_union_percent }}';
            @endif
            updateTradeUnionInputLabel();
        @endif

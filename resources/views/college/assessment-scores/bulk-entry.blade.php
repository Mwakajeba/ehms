...
@section('title', 'Bulk Assessment Score Entry')
...
<div class="container-fluid py-4">
    ...
    <div class="breadcrumb-nav">
        ...
        <span>/</span>
        <a href="{{ route('college.assessment-scores.index') }}">Assessment Scores</a>
        <span>/</span>
    ...
    <div class="page-header">
        <h1><i class='bx bx-list-plus'></i> Bulk Assessment Score Entry</h1>
        <p>Enter scores for all students in a class at once</p>
    </div>
    ...
    <div class="filter-card">
        <h3><i class='bx bx-filter-alt'></i> Select Assessment</h3>
        <form id="filterForm">
            <div class="filter-grid">
                ...
                <div class="filter-group">
                    ...
                    <select name="course_assessment_id" id="course_assessment_id" required>
    ...
    <div class="scores-card" id="scoresCard" style="display: none;">
        <form id="bulkScoresForm" action="{{ route('college.assessment-scores.store-bulk') }}" method="POST">
            @csrf
            <input type="hidden" name="course_assessment_id" id="form_course_assessment_id">

            <div class="scores-card-header">
                <h3><i class='bx bx-user-check'></i> Student Score Entry</h3>
            ...
            <div class="empty-state" id="emptyState" style="display: none;">
                ...
                <p>No students are registered for this course assessment.</p>
            </div>
            ...
            <div class="form-actions">
                ...
                <div class="form-actions-right">
                    <a href="{{ route('college.assessment-scores.index') }}" class="btn btn-secondary">
    ...
    <div class="alert alert-info" id="initialMessage">
        ...
        <span>Select a program, course, and assessment above to load students for score entry.</span>
    </div>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const assessmentSelect = document.getElementById('course_assessment_id');
            ...
            fetch(`/college/api/course-assessments?course_id=${courseId}&academic_year_id=${academicYearId}&semester_id=${semesterId}`)
                ...
                .then(data => {
                    ...
                    data.forEach(assessment => {
                        assessmentSelect.innerHTML += `<option value="${assessment.id}" data-max-score="${assessment.max_score}" data-name="${assessment.name} (${assessment.type})">${assessment.name} (${assessment.type}) - Max: ${assessment.max_score}</option>`;
                    });
                })
                .catch(() => {
    ...
    filterForm.addEventListener('submit', function(e) {
        ...
        const assessmentName = selectedOption.dataset.name || 'Assessment';

        document.getElementById('assessmentName').textContent = assessmentName;
        document.getElementById('maxScore').textContent = maxScore;
        document.getElementById('form_course_assessment_id').value = assessmentId;

        // Load students
        studentsTableBody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem;">Loading students...</td></tr>';
        scoresCard.style.display = 'block';
        initialMessage.style.display = 'none';

        fetch(`/college/api/assessment-students?course_assessment_id=${assessmentId}`)
            .then(response => response.json())
            .then(data => {
                if (data.length === 0) {
            ...
            .catch(error => {
                studentsTableBody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 2rem; color: #ef4444;">Failed to load students. Please try again.</td></tr>';

...
@section('title', 'Continuous Assessment Scores')
...
<div class="results-dashboard">
    ...
    <nav class="breadcrumb-nav">
        ...
        <span class="breadcrumb-btn active">
            <i class='bx bx-edit'></i> CA Scores
        </span>
    </nav>

    <!-- Page Header -->
    <div class="page-header">
        <h1><i class='bx bx-edit'></i> Continuous Assessment Scores</h1>
        <p>Manage scores for tests, quizzes, midterms, assignments, and other continuous assessments</p>
    </div>

    <!-- Statistics Row -->
    <div class="stats-row">
        <div class="stat-card green">
            <div class="stat-icon green"><i class='bx bx-file'></i></div>
        ...
        <div class="stat-card blue">
            <div class="stat-icon blue"><i class='bx bx-check'></i></div>
            <div class="stat-content">
                <h4>Marked</h4>
    ...
    <div class="filter-card">
        ...
        <form action="{{ route('college.assessment-scores.index') }}" method="GET">
    ...
    <div class="data-card">
        <div class="data-card-header">
            <h3><i class='bx bx-list-ul'></i> Assessment Score Records</h3>
            <a href="{{ route('college.assessment-scores.create') }}" class="btn btn-primary">
                    ...
                    <tbody>
                        @foreach($scores as $score)
                            <tr>
                                ...
                                <td>
                                    <strong>{{ $score->courseAssessment->title ?? 'N/A' }}</strong>
                                    <br>
                                    <small class="text-muted">{{ $score->courseAssessment->assessmentType->name ?? '' }}</small>
                                </td>
                                <td>{{ $score->courseAssessment->course->code ?? '' }} - {{ $score->courseAssessment->course->name ?? '' }}</td>
                                <td>
                                    @php
                                    ...
                                    <span class="score-badge {{ $scoreClass }}">
                                        {{ $score->score }}/{{ $score->max_marks }}
                                    </span>
                                </td>
                                <td>{{ number_format($score->weighted_score, 2) }}%</td>
                                <td>
                                    <span class="status-badge {{ $score->status }}">
                                        <i class='bx bx-{{ $score->status == 'published' ? 'check-circle' : ($score->status == 'marked' ? 'check' : 'time') }}'></i>
                                        {{ ucfirst($score->status) }}
                                    </span>
                                </td>
                                <td>
                                    <div class="action-btns">
                                        <a href="{{ route('college.assessment-scores.show', $score) }}" class="action-btn view" title="View">
                                            <i class='bx bx-show'></i>
                                        </a>
                                        @if($score->status !== 'published')
                                            <a href="{{ route('college.assessment-scores.edit', $score) }}" class="action-btn edit" title="Edit">
                                                <i class='bx bx-edit'></i>
                                            </a>
                                            <form action="{{ route('college.assessment-scores.publish', $score) }}" method="POST" style="display:inline;">
                                                @csrf
        ...
        @else
            <div class="empty-state">
                ...
                <h3>No Assessment Scores Found</h3>
                <p>No scores match your current filter criteria. Try adjusting your filters or add new scores.</p>
                <a href="{{ route('college.assessment-scores.create') }}" class="btn btn-primary" style="margin-top: 15px;">
                    <i class='bx bx-plus'></i> Enter Scores
                </a>
            </div>
        @endif

                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-primary">Edit Department: {{ $department->name }}</h5>
                        </div>
                        <hr />

                        <form action="{{ route('college.departments.update', $department->id) }}" method="POST" id="departmentForm">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="card-body">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input" type="checkbox"
                                                           id="is_active" name="is_active" value="1" {{ old('is_active', $department->is_active) ? 'checked' : '' }}>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror"
                                                  ...
                                                  placeholder="Optional description of the department">{{ old('description', $department->description) }}</textarea>
                                        @error('description')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                        <div class="form-text text-muted">
                                    ...
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="d-flex gap-2">
                                            ...
                                            <button type="submit" class="btn btn-primary">
                                                <i class="bx bx-save me-1"></i> Update Department
                                            </button>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="alert alert-info">
                            <h6><i class="bx bx-bulb me-1"></i> Tips for Editing Departments:</h6>
                            <ul class="mb-0 small">
                                <li>Department names should remain descriptive and professional</li>
                                <li>Changing codes may affect existing records</li>
                                <li>Update head of department as needed for organizational changes</li>
                                <li>Use description to reflect current department objectives</li>
                            </ul>
                        </div>
                        ...
                        <div class="mb-3">
                            <strong>Created:</strong> {{ $department->created_at->format('M d, Y H:i') }}
                        </div>
                        <div class="mb-3">
                            <strong>Last Updated:</strong> {{ $department->updated_at->format('M d, Y H:i') }}
                        </div>
                        @if($department->programs->count() > 0)
                        <div class="mb-3">
                            <strong>Programs:</strong> {{ $department->programs->count() }}
                            <ul class="small text-muted mt-1">
                                @foreach($department->programs->take(3) as $program)
                                    <li>{{ $program->name }}</li>
                                @endforeach
                                @if($department->programs->count() > 3)
                                    <li><em>and {{ $department->programs->count() - 3 }} more...</em></li>
                                @endif
                            </ul>
                        </div>
                        @endif
                        <div class="alert alert-warning">
                            ...
                            <strong>Warning:</strong> Changing the department code may affect existing records and integrations.
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    console.log('Enhanced college department edit form loaded');
...
function resetForm() {
    if (confirm('Are you sure you want to reset the form? All changes will be lost.')) {
        ...
        // Reset Select2
        $('#head_of_department_id').val('{{ $department->head_of_department_id }}').trigger('change');

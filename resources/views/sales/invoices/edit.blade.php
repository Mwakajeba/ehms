        ...
        <div class="card">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bx bx-receipt me-2"></i>New Sales Invoice</h5>
                    ...
                    <div class="card-body">
                <form id="invoice-form" method="POST" action="{{ route('sales.invoices.update', $invoice->encoded_id) }}">
                    @method('PUT')
                            @csrf
...
$(document).ready(function() {
    ...
    function addExistingItemToTable(item, index) {
        ...
        const invoiceExchangeRate = parseFloat('{{ $invoice->exchange_rate ?? 1 }}');
        ...
        let displayPrice = item.unitPrice; // Default to stored price
        ...
        const discountDisplay = item.discountType === 'percentage' ? `${item.discountRate}%` : `TSh ${item.discountRate.toFixed(2)}`;
        const vatDisplay = item.vatType === 'no_vat' ? 'No VAT' : `${item.vatRate}%`;
        ...
        const row = `
            <tr data-item-id="${item.id}">
                <td>
                    ...
                    <input type="hidden" name="items[${index}][vat_type]" value="${item.vatType}">
                    <input type="hidden" name="items[${index}][vat_rate]" value="${item.vatRate}">
                    <input type="hidden" name="items[${index}][discount_type]" value="${item.discountType}">
                    <input type="hidden" name="items[${index}][discount_rate]" value="${item.discountRate}">
                    <input type="hidden" name="items[${index}][notes]" value="${item.notes}">
                    <input type="hidden" name="items[${index}][line_total]" value="${item.lineTotal}">
                    <input type="hidden" name="items[${index}][vat_amount]" value="${item.vatAmount || 0}">
                ...
                <td>
                    <small class="text-muted">${(item.vatType === 'no_vat') ? 'No VAT' : ((item.vatType === 'exclusive' ? 'Exclusive' : 'Inclusive') + ' (' + item.vatRate + '%) - ' + (displayPrice * item.quantity * (item.vatType === 'exclusive' ? (item.vatRate/100) : (item.vatRate/(100+item.vatRate)))).toFixed(2))}</small>
                </td>
                <td>
                    <span class="line-total">${calculateLineTotal(displayPrice, item.quantity, item.vatType, item.vatRate).toFixed(2)}</span>
                </td>
                <td>
                    <button type="button" class="btn btn-outline-danger btn-sm remove-item">
                        <i class="bx bx-trash"></i>

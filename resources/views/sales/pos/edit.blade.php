		...
		tr.innerHTML = `
			<td>
				<select name="items[${index}][item_id]" class="form-select item-select" required>
					<option value="">Select item</option>
					@foreach($inventoryItems as $inv)
						<option value="{{ $inv->id }}" data-price="{{ $inv->unit_price }}" data-vat-rate="{{ $inv->vat_rate ?? 18 }}" data-vat-type="{{ $inv->vat_type ?? 'inclusive' }}">{{ $inv->name }} ({{ $inv->code }}) - TZS {{ number_format($inv->unit_price ?? 0, 2) }}</option>
					@endforeach
				</select>
			</td>

...
@section('title', 'Bulk Payment - ' . $student->first_name . ' ' . $student->last_name . ' - ' . $quarter)
...
<div class="page-wrapper">
    <div class="page-content">
        <x-breadcrumbs-with-icons :links="[
            ...
            ['label' => 'Bulk Payment - ' . $quarter, 'url' => '#', 'icon' => 'bx bx-plus-circle']
        ]" />

        <h6 class="mb-0 text-uppercase">BULK PAYMENT - {{ $student->first_name . ' ' . $student->last_name }} - {{ $quarter }}</h6>
        <hr />
        ...
        <div class="card shadow-sm">
            <div class="card-header bg-success text-white">
                <h5 class="card-title mb-0">
                    <i class="bx bx-plus-circle me-2"></i>Record Bulk Payment
                </h5>
            </div>
            <div class="card-body">
                    <form method="POST" action="{{ route('school.fee-invoices.bulk-payment.store', ['encodedId' => $student->getRouteKey(), 'quarter' => $quarter]) }}">
                    @csrf
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('amount') is-invalid @enderror"
                                           ...
                                           value="{{ old('amount', number_format($quarterInvoices->where('status', '!=', 'paid')->sum('total_amount'), 2, '.', '')) }}" required>
                                </div>
                                @error('amount')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror

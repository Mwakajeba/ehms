...
<head>
    <meta charset="utf-8">
    <title>Class Workload Balance Report</title>
    <style>
...
<body>
    <div class="header">
        <h1>CLASS WORKLOAD BALANCE REPORT</h1>
        @if($company)
        ...
        <tbody>
            @foreach($data as $row)
                <tr>
                    ...
                    <td class="text-center">{{ $row['total_estimated_time'] }}</td>
                    ...
                    <td>{{ $row['subjects'] }}</td>
                    <td class="text-center">{{ $row['workload_status'] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>

...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">EDIT FEE GROUP</h6>
        <hr />
        ...
        <div class="row">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-warning">Edit Fee Group</h5>
                        </div>
                        <hr />

                        <form action="{{ route('school.fee-groups.update', $feeGroup->hashid) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('fee_code') is-invalid @enderror" id="fee_code" name="fee_code" value="{{ old('fee_code', $feeGroup->fee_code) }}" placeholder="e.g., TUITION, EXAM, LAB" required>
                                        @error('fee_code')
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $feeGroup->name) }}" placeholder="e.g., Tuition Fees, Examination Fees" required>
                                        @error('name')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control chart-account-select @error('receivable_account_id') is-invalid @enderror" id="receivable_account_id" name="receivable_account_id" required>
                                            ...
                                            @foreach($chartAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('receivable_account_id', $feeGroup->receivable_account_id) == $account->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control chart-account-select @error('income_account_id') is-invalid @enderror" id="income_account_id" name="income_account_id" required>
                                            ...
                                            @foreach($chartAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('income_account_id', $feeGroup->income_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control chart-account-select @error('transport_income_account_id') is-invalid @enderror" id="transport_income_account_id" name="transport_income_account_id">
                                            ...
                                            @foreach($chartAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('transport_income_account_id', $feeGroup->transport_income_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control chart-account-select @error('discount_account_id') is-invalid @enderror" id="discount_account_id" name="discount_account_id">
                                            ...
                                            @foreach($chartAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('discount_account_id', $feeGroup->discount_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-control chart-account-select @error('opening_balance_account_id') is-invalid @enderror" id="opening_balance_account_id" name="opening_balance_account_id">
                                            ...
                                            @foreach($chartAccounts as $account)
                                                <option value="{{ $account->id }}" {{ old('opening_balance_account_id', $feeGroup->opening_balance_account_id) == $account->id ? 'selected' : '' }}>
                                                    {{ $account->account_code }} - {{ $account->account_name }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3" placeholder="Optional description of the fee group">{{ old('description', $feeGroup->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $feeGroup->is_active) ? 'checked' : '' }}>
                            ...
                            <div class="d-flex justify-content-between mt-4">
                                ...
                                <button type="submit" class="btn btn-warning">
                                    <i class="bx bx-save me-1"></i> Update Fee Group
                                </button>
                            </div>
                        </form>
                    </div>
            ...
            <div class="col-12 col-lg-4">
                <div class="card">
                    <div class="card-body">
                        <h6 class="card-title">
                            <i class="bx bx-info-circle me-1 text-info"></i> Fee Group Information
                        </h6>
                        <hr />
                        <div class="mb-3">
                            <h6>Fee Group Details:</h6>
                            <div class="small text-muted">
                                <p><strong>Created:</strong> {{ $feeGroup->created_at->format('M d, Y H:i') }}</p>
                                <p><strong>Last Updated:</strong> {{ $feeGroup->updated_at->format('M d, Y H:i') }}</p>
                                @if($feeGroup->creator)
                                    <p><strong>Created By:</strong> {{ $feeGroup->creator->name }}</p>
                                @endif
                            </div>
                        </div>
                        <div class="alert alert-light small">
                            <i class="bx bx-bulb me-1 text-warning"></i>
                            <strong>Tip:</strong> Changes to chart accounts may affect existing fee structures and financial reporting.
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        console.log('Edit fee group form loaded with Select2');
    });
</script>
@endpush
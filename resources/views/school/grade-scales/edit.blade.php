...
<div class="page-wrapper">
    <div class="page-content">
        ...
        <h6 class="mb-0 text-uppercase">EDIT GRADE SCALE</h6>
        <hr />
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="card-title d-flex align-items-center">
                            ...
                            <h5 class="mb-0 text-danger">Edit Grade Scale</h5>
                        </div>
                        <hr />

                        <form id="grade-scale-form" action="{{ route('school.grade-scales.update', $gradeScale->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="card border-warning">
                                        ...
                                        <div class="card-body">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('name', $gradeScale->name) }}" required>
                                                @error('name')
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select @error('academic_year_id') is-invalid @enderror" id="academic_year_id" name="academic_year_id" required>
                                                    ...
                                                    @foreach($academicYears as $year)
                                                        <option value="{{ $year->id }}" {{ old('academic_year_id', $gradeScale->academic_year_id) == $year->id ? 'selected' : '' }}>
                                                            {{ $year->year_name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select class="form-select @error('max_marks') is-invalid @enderror" id="max_marks" name="max_marks" required>
                                                    ...
                                                    <option value="50" {{ old('max_marks', $gradeScale->max_marks) == '50' ? 'selected' : '' }}>50 Marks</option>
                                                    <option value="100" {{ old('max_marks', $gradeScale->max_marks) == '100' ? 'selected' : '' }}>100 Marks</option>
                                                </select>
                                                @error('max_marks')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <input type="number" step="0.01" class="form-control @error('passed_average_point') is-invalid @enderror" id="passed_average_point" name="passed_average_point" value="{{ old('passed_average_point', $gradeScale->passed_average_point) }}" required>
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="3">{{ old('description', $gradeScale->description) }}</textarea>
                                                @error('description')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                            ...
                                            <div class="mb-3">
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $gradeScale->is_active) ? 'checked' : '' }}>
                                    ...
                                    <div class="card border-info">
                                        ...
                                        <div class="card-body">
                                            <div id="grades-container">
                                                <!-- Existing grades will be loaded here -->
                                                @foreach($gradeScale->grades->sortBy('sort_order') as $index => $grade)
                                                    <div class="grade-row" data-index="{{ $index }}">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-6">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="text" class="form-control" name="grades[{{ $index }}][grade_letter]" value="{{ old('grades.' . $index . '.grade_letter', $grade->grade_letter) }}" required maxlength="5">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-6">
                                                                <div class="mb-3">
                                                                    <label class="form-label">Grade Name <span class="text-danger">*</span></label>
                                                                    <input type="text" class="form-control" name="grades[{{ $index }}][grade_name]" value="{{ old('grades.' . $index . '.grade_name', $grade->grade_name) }}" required>
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-md-3">
                                                                <div class="mb-3">
                                                                    ...
                                                                    <input type="number" class="form-control" name="grades[{{ $index }}][min_marks]" value="{{ old('grades.' . $index . '.min_marks', $grade->min_marks) }}" required min="0" step="0.01">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3">
                                                                <div class="mb-3">
                                                                    <label class="form-label">Max Marks <span class="text-danger">*</span></label>
                                                                    <input type="number" class="form-control" name="grades[{{ $index }}][max_marks]" value="{{ old('grades.' . $index . '.max_marks', $grade->max_marks) }}" required min="0" step="0.01">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3">
                                                                <div class="mb-3">
                                                                    <label class="form-label">Grade Point</label>
                                                                    <input type="number" class="form-control" name="grades[{{ $index }}][grade_point]" value="{{ old('grades.' . $index . '.grade_point', $grade->grade_point) }}" min="0" step="0.01">
                                                                </div>
                                                            </div>
                                                            <div class="col-md-3">
                                                                <div class="mb-3">
                                                                    <label class="form-label">Sort Order</label>
                                                                    <input type="number" class="form-control" name="grades[{{ $index }}][sort_order]" value="{{ old('grades.' . $index . '.sort_order', $grade->sort_order) }}" min="1">
                                                                </div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <textarea class="form-control" name="grades[{{ $index }}][remarks]" rows="2">{{ old('grades.' . $index . '.remarks', $grade->remarks) }}</textarea>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                            ...
                            <div class="row mt-4">
                                <div class="col-12">
                                    <div class="d-flex justify-content-between">
                                        <a href="{{ route('school.grade-scales.show', $gradeScale->id) }}" class="btn btn-info">
                                            <i class="bx bx-show me-1"></i>View
                                        </a>
                                        <div>
                                            <a href="{{ route('school.grade-scales.index') }}" class="btn btn-secondary me-2">
                                                <i class="bx bx-arrow-back me-1"></i>Back to Grade Scales
                                            </a>
                                            <button type="submit" class="btn btn-danger">
                                                <i class="bx bx-save me-1"></i>Update Grade Scale
                                            </button>
                                        </div>
                                    </div>
                                </div>
...
$(document).ready(function() {
    let gradeIndex = {{ $gradeScale->grades->count() }};

    // Calculate the next sort order
    let nextSortOrder = 1;
    $('.grade-row input[name*="[sort_order]"]').each(function() {
        const currentOrder = parseInt($(this).val()) || 0;
        if (currentOrder >= nextSortOrder) {
            nextSortOrder = currentOrder + 1;
        }
    });

    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumbs-with-icons :links="[
                ...
                ['label' => 'RCH', 'url' => '#', 'icon' => 'bx bx-heart']
            ]" />
            <h6 class="mb-0 text-uppercase">RCH DASHBOARD</h6>
            <hr />
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
                                    <i class="bx bx-bar-chart me-2"></i>Services Today by Type
                                </h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                        ...
                        <div class="card-body">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach($waitingVisits as $visit)
                                                @php
                                                    $rchDept = $visit->visitDepartments->firstWhere('department.type', 'rch');
                                                    $waitingTime = $rchDept && $rchDept->waiting_started_at 
                                                        ? $rchDept->waiting_started_at->diffForHumans() 
                                                        : 'N/A';
                                                @endphp
                                                <tr>
                                                    ...
                                                    <td>{{ ucfirst($visit->patient->gender ?? 'N/A') }}</td>
                                                    <td>{{ $waitingTime }}</td>
                                                    <td>
                                                        <form action="{{ route('hospital.rch.start-service', $visit->id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-primary">
                                                                <i class="bx bx-play me-1"></i>Start Service
                                                            </button>
                                                        </form>
                                                        <a href="{{ route('hospital.rch.create', $visit->id) }}" class="btn btn-sm btn-info">
                            ...
                            @else
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted mt-2">No patients waiting for RCH services.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach($inServiceVisits as $visit)
                                                @php
                                                    $rchDept = $visit->visitDepartments->firstWhere('department.type', 'rch');
                                                    $serviceTime = $rchDept && $rchDept->service_started_at 
                                                        ? $rchDept->service_started_at->diffForHumans() 
                                                        : 'N/A';
                                                @endphp
                                                <tr>
                                                    <td><strong>{{ $visit->visit_number }}</strong></td>
                                                    <td>{{ $visit->patient->full_name }}</td>
                                                    <td>{{ $visit->patient->mrn }}</td>
                                                    <td>{{ $rchDept && $rchDept->service_started_at ? $rchDept->service_started_at->format('H:i') : 'N/A' }}</td>
                                                    <td>{{ $serviceTime }}</td>
                                                    <td>
                                                        <span class="badge bg-info">{{ $visit->rchRecords->count() }} record(s)</span>
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('hospital.rch.create', $visit->id) }}" class="btn btn-sm btn-info">
                                        ...
                                        <tbody>
                                            @foreach($followUpRequired as $record)
                                                <tr>
                                                    ...
                                                    <td>{{ ucfirst(str_replace('_', ' ', $record->service_type)) }}</td>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('hospital.rch.show', $record->id) }}" class="btn btn-sm btn-info">

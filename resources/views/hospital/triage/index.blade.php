    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumbs-with-icons :links="[
                ...
                ['label' => 'Triage', 'url' => '#', 'icon' => 'bx bx-pulse']
            ]" />
            <h6 class="mb-0 text-uppercase">TRIAGE DASHBOARD</h6>
            <hr />
                        ...
                        <div class="card-body">
                                        ...
                                        <tbody>
                                            @foreach($waitingVisits as $visit)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $waitingTime }}</td>
                                                    <td>
                                                        <form action="{{ route('hospital.triage.start-service', $visit->id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-primary">
                                                                <i class="bx bx-play me-1"></i>Start Service
                                                            </button>
                                                        </form>
                                                        <a href="{{ route('hospital.triage.create', $visit->id) }}" class="btn btn-sm btn-info">
                            ...
                            @else
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted mt-2">No patients waiting for triage.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                                        ...
                                        <tbody>
                                            @foreach($inServiceVisits as $visit)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{ $visit->patient->mrn }}</td>
                                                    <td>{{ $visit->patient->age ? $visit->patient->age . ' years' : 'N/A' }}</td>
                                                    <td>{{ $triageDept && $triageDept->service_started_at ? $triageDept->service_started_at->format('H:i') : 'N/A' }}</td>
                                                    <td>{{ $serviceTime }}</td>
                                                    <td>{{ $triageDept && $triageDept->servedBy ? $triageDept->servedBy->name : 'N/A' }}</td>
                                                    <td>
                                                        ...
                                                        @else
                                                            <a href="{{ route('hospital.triage.show', $visit->id) }}" class="btn btn-sm btn-success">
                                                                <i class="bx bx-show me-1"></i>View Vitals
                                                            </a>
                                                        @endif
                                                    </td>
                                                </tr>

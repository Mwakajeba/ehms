...
@section('title', 'Dental Dashboard')
    ...
    <div class="page-wrapper">
        <div class="page-content">
            <x-breadcrumbs-with-icons :links="[
                ...
                ['label' => 'Dental', 'url' => '#', 'icon' => 'bx bx-smile']
            ]" />
            <h6 class="mb-0 text-uppercase">DENTAL DASHBOARD</h6>
            <hr />
                        ...
                        <div class="card-body">
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach($waitingVisits as $visit)
                                                @php
                                                    $dentalDept = $visit->visitDepartments->firstWhere('department.type', 'dental');
                                                    $waitingTime = $dentalDept && $dentalDept->waiting_started_at 
                                                        ? $dentalDept->waiting_started_at->diffForHumans() 
                                                        : 'N/A';
                                                @endphp
                                                <tr>
                                                    ...
                                                    <td>{{ $waitingTime }}</td>
                                                    <td>
                                                        <form action="{{ route('hospital.dental.start-service', $visit->id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-primary">
                                                                <i class="bx bx-play me-1"></i>Start Service
                                                            </button>
                                                        </form>
                                                        <a href="{{ route('hospital.dental.create', $visit->id) }}" class="btn btn-sm btn-info">
                                                            <i class="bx bx-plus me-1"></i>Record Procedure
                                                        </a>
                                                    </td>
                                                </tr>
                                            @endforeach
                            ...
                            @else
                                <div class="text-center py-4">
                                    ...
                                    <p class="text-muted mt-2">No patients waiting for dental procedures.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach($inServiceVisits as $visit)
                                                @php
                                                    $dentalDept = $visit->visitDepartments->firstWhere('department.type', 'dental');
                                                    $serviceTime = $dentalDept && $dentalDept->service_started_at 
                                                        ? $dentalDept->service_started_at->diffForHumans() 
                                                        : 'N/A';
                                                @endphp
                                                <tr>
                                                    <td><strong>{{ $visit->visit_number }}</strong></td>
                                                    <td>{{ $visit->patient->full_name }}</td>
                                                    <td>{{ $visit->patient->mrn }}</td>
                                                    <td>{{ $dentalDept && $dentalDept->service_started_at ? $dentalDept->service_started_at->format('H:i') : 'N/A' }}</td>
                                                    <td>{{ $serviceTime }}</td>
                                                    <td>
                                                        <span class="badge bg-info">{{ $visit->dentalRecords->count() }} record(s)</span>
                                                    </td>
                                                    <td>
                                                        <a href="{{ route('hospital.dental.create', $visit->id) }}" class="btn btn-sm btn-info">
                                                            <i class="bx bx-plus me-1"></i>Add Procedure
                                                        </a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        ...
                                        <tbody>
                                            @foreach($followUpRequired as $record)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('hospital.dental.show', $record->id) }}" class="btn btn-sm btn-info">

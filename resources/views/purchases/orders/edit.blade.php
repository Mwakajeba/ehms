...
<div class="page-wrapper">
    <div class="page-content">
		...
		<div class="card">
			...
			<div class="card-body">
				<form id="order-form" action="{{ route('purchases.orders.update', $order->encoded_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
								...
								<table class="table" id="items-table">
									<thead>
                                        <tr>
                                            ...
                                            <th width="14%">Cost Price</th>
                                            <th width="14%">VAT</th>
                                            <th width="14%">VAT Amount</th>
									...
									<tbody id="items-tbody">
										...
										@foreach($order->items as $item)
											<tr data-item-id="{{ $item->item_id }}" data-row-id="{{ $row }}">
												<td>
													<strong>{{ $item->item->name }}</strong><br>
													<small class="text-muted">{{ $item->item->code }}</small>
													<input type="hidden" name="items[{{ $row }}][item_id]" value="{{ $item->item_id }}">
                                                    <input type="hidden" name="items[{{ $row }}][vat_type]" value="{{ $item->vat_type === 'inclusive' ? 'vat_inclusive' : ($item->vat_type === 'exclusive' ? 'vat_exclusive' : 'no_vat') }}">
													<input type="hidden" name="items[{{ $row }}][vat_rate]" value="{{ $item->vat_rate }}">
													<input type="hidden" name="items[{{ $row }}][vat_amount]" value="{{ $item->vat_amount }}">
													<input type="hidden" name="items[{{ $row }}][total_amount]" value="{{ number_format($item->total_amount, 2, '.', '') }}">
                                                </td>
                                                <td>
												<input type="number" class="form-control item-quantity" name="items[{{ $row }}][quantity]" value="{{ number_format($item->quantity, 2, '.', '') }}" step="0.01" min="0.01" data-row="{{ $row }}">
                                                </td>
                                                <td>
												<input type="number" class="form-control item-price" name="items[{{ $row }}][cost_price]" value="{{ number_format($item->item->cost_price, 2, '.', '') }}" step="0.01" min="0" data-row="{{ $row }}">
                                                </td>
                                            <td>
